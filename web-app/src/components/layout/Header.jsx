...
const mobileNavLinks = [
  { label: 'THƯƠNG HIỆU', href: '/category' },
  { label: 'TẤT CẢ SẢN PHẨM', href: '/category' },
  { label: 'SẢN PHẨM MỚI', href: '/new-arrivals' },
  { label: 'NAM', href: '/nam' },
  { label: 'NỮ', href: '/nu' },
  { label: 'TRẺ EM', href: '/tre-em' },
...
export default function Header() {
  ...
  // State lưu tất cả sản phẩm
  const [allProducts, setAllProducts] = useState([]);

  // Gọi API lấy TẤT CẢ Sản phẩm 1 lần duy nhất (Dùng chung cho Tìm kiếm & Tạo Menu)
  useEffect(() => {
  ...
  /* ── TẠO MENU DANH MỤC ĐỘNG "REAL" TỪ SẢN PHẨM ─────────────── */
  const sportsCategories = useMemo(() => {
    if (!allProducts || allProducts.length === 0) return [];

    // Rút trích loại sản phẩm (VD: Quần áo chạy bộ, Vợt cầu lông...)
    const types = allProducts.map(p => p.productType).filter(Boolean);
    // Xóa trùng lặp
    const uniqueTypes = [...new Set(types)].sort();

    // Tạo mảng category để render ra HTML
    return uniqueTypes.map(type => ({
      id: createSlug(type),
      label: type.toUpperCase(), // Viết hoa cho đẹp
      href: `/category/${createSlug(type)}` // Trỏ tới đúng Route động chúng ta đã làm
    }));
  }, [allProducts]);
  ...
  return (
      <header className="header-outer">
              ...
              <li className="nav-list-item nav-has-dropdown">
                ...
                <ul className="dropdown-menu" role="menu">
                  {sportsCategories.length === 0 ? (
                      <li role="none"><span className="dropdown-item" style={{ color: '#999', pointerEvents: 'none' }}>Đang tải...</span></li>
                  ) : (
                      sportsCategories.map((cat) => (
                          <li key={cat.id} role="none">
                            <a href={cat.href} role="menuitem" className="dropdown-item">
                              {cat.label}
                            </a>
                          </li>
                      ))
        ...
        >
          ...
          {/* DANH MỤC TRÊN ĐIỆN THOẠI CŨNG REAL LUÔN */}
          <div className="mobile-nav-group">
            <span className="mobile-nav-item mobile-nav-group-title">DANH MỤC</span>
            {sportsCategories.map((cat) => (
                <a
                    key={cat.id}
                    href={cat.href}
